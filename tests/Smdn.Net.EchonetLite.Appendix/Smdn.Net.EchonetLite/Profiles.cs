...
public class ProfilesTests {
  ...
  {
    var p = Profiles.ノードプロファイル;

    Assert.That(p, Is.Not.Null);
    Assert.That(p.ClassGroup, Is.Not.Null, nameof(IEchonetObject.ClassGroup));
    Assert.That(p.ClassGroup.ClassGroupCode, Is.EqualTo(0x0E), nameof(EchonetClassGroupSpecification.ClassGroupCode));
    Assert.That(p.ClassGroup.ClassGroupNameOfficial, Is.EqualTo("プロファイルクラスグループ"), nameof(EchonetClassGroupSpecification.ClassGroupNameOfficial));
    Assert.That(p.ClassGroup.SuperClass, Is.EqualTo("プロファイルオブジェクトスーパークラス"), nameof(EchonetClassGroupSpecification.SuperClass));

    Assert.That(p.ClassGroup.ClassList, Is.Not.Null, nameof(EchonetClassGroupSpecification.ClassList));
    Assert.That(p.ClassGroup.ClassList.Count, Is.EqualTo(1), nameof(EchonetClassGroupSpecification.ClassList));

    Assert.That(p.ClassGroup.ClassList[0].ClassCode, Is.EqualTo(0xF0), nameof(EchonetClassGroupSpecification.ClassList));

    var c = p.ClassGroup.ClassList[0];

    Assert.That(c.ClassCode, Is.EqualTo(0xF0), nameof(EchonetClassSpecification.ClassCode));
    Assert.That(c.ClassNameOfficial, Is.EqualTo("ノードプロファイル"), nameof(EchonetClassSpecification.ClassNameOfficial));
  ...
  {
    var epc88 = Profiles.ノードプロファイル.GetProperties.FirstOrDefault(static prop => prop.Name == "異常発生状態");
    ...
    var epc9D = Profiles.ノードプロファイル.GetProperties.FirstOrDefault(static prop => prop.Code == 0x9D);
  ...
  {
    var epc82 = Profiles.ノードプロファイル.GetProperties.FirstOrDefault(static prop => prop.Code == 0x82);
    ...
    var epcD3 = Profiles.ノードプロファイル.GetProperties.FirstOrDefault(static prop => prop.Code == 0xD3);
